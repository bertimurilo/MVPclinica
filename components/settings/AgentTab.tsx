...
const INPUT_CLS =
  'w-full bg-gray-900 border border-gray-700 text-white rounded-lg px-3.5 py-2.5 text-sm placeholder:text-gray-600 outline-none focus:ring-2 focus:ring-violet-500/30 focus:border-violet-500 transition-all resize-none'
    ...
    <form onSubmit={handleSubmit} className="space-y-5 max-w-2xl">
          ...
          {TONES.map(t => (
            <label key={t.value} className="relative cursor-pointer">
              ...
              <div className={`p-3.5 rounded-xl border transition-all ${
                ...
                <p className={`text-sm font-semibold ${tone === t.value ? 'text-violet-400' : 'text-white'}`}>
                  {t.label}
                </p>
          ...
          {[
            ...
            <label key={rule.name} className="flex items-start gap-3 cursor-pointer group">
              <input
                ...
                className="mt-0.5 w-4 h-4 rounded border-gray-600 bg-gray-900 text-violet-500 focus:ring-violet-500/30 shrink-0"
              />
              <div>
              ...
              {enabledDays[day.key] ? (
                <div className="flex items-center gap-2 pl-12 md:pl-0">
                  <input
                    ...
                    className="bg-gray-900 border border-gray-700 text-white rounded-lg px-2.5 py-1.5 text-sm outline-none focus:ring-2 focus:ring-violet-500/30 focus:border-violet-500"
                  />
      ...
      <Section title="Límite de mensajes automáticos">
        <div className="flex items-center gap-4">
          <input
            ...
            className="w-24 bg-gray-900 border border-gray-700 text-white rounded-lg px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-violet-500/30 focus:border-violet-500"
          />
      ...
      <div className="flex items-center gap-3">
        <button
          type="submit"
          disabled={isPending}
          className="px-4 py-2 text-sm font-medium rounded-lg bg-violet-600 hover:bg-violet-700 text-white transition-colors disabled:opacity-50 flex items-center gap-2"
        >
